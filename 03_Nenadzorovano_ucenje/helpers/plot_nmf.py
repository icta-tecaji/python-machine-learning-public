...
def plot_nmf_illustration():
    ...
    nmf = NMF(init='random', random_state=0, max_iter=10000)
    nmf.fit(X_blob)
    ...
    # second plot
    nmf = NMF(init='random', max_iter=10000, random_state=0, n_components=1)
    nmf.fit(X_blob)
...
def nmf_faces(X_train, X_test):
    ...
    for n_components in [10, 50, 100, 500]:
        # build the NMF model
        nmf = NMF(init='random', n_components=n_components, random_state=0, max_iter=10000)
        nmf.fit(X_train)
